...
/**
 * Forked from the same class from Keycloak
 *
 * @author rmartinc
 */
public class AuthzTestUtils {
    ...
    public static InputStream httpsAwareConfigurationStream(InputStream input) throws IOException {
        ...
        try (PrintWriter pw = new PrintWriter(out);
            ...
            while (s.hasNextLine()) {
                String lineWithReplaces = s.nextLine().replace("http://localhost:8180",
                        ServerURLs.AUTH_SERVER_SCHEME + "://localhost:" + ServerURLs.AUTH_SERVER_PORT);
    ...
    public static PolicyEnforcer createPolicyEnforcer(String resource, boolean bearerOnly) {
        try (InputStream is = getAdapterConfiguration(resource)) {
            return PolicyEnforcer.builder()
                    ...
                    .httpClient(TestRegistry.INSTANCE.getOrCreateProvider(CloseableHttpClient.class))
                    .build();
        } catch (IOException e) {
